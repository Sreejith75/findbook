...
1. Set `FindBook.AspireHost` as the startup project
...
```bash
dotnet ef migrations add MigrationName -c AppDbContext -p ../FindBook.Infrastructure/FindBook.Infrastructure.csproj -s FindBook.Web.csproj -o Data/Migrations
```

To update the database:
```bash
dotnet ef database update -c AppDbContext -p ../FindBook.Infrastructure/FindBook.Infrastructure.csproj -s FindBook.Web.csproj
```
